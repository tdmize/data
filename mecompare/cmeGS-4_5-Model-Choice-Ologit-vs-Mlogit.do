capture log close
log using cmeGS-4_5-Model-Choice-Ologit-vs-Mlogit, replace text

// CME: Comparing Marginal Effects Across Models
// Mize, Doan, and Long
// Model Selection: Ex 4.5 Ordinal logit and multinomial logit

// NOTE_0: This program can take 5 or more minutes to complete
...
mlincom, stat(est se p) dec(3) twidth(15) ///
        title("DCR for age 50 to 60 Across OLM and MNL")

		
****************************************************************        
// #4 Plots of predicted probabilities
****************************************************************        
*NOTE_3: Graphs in the paper use the cleanplots graphics scheme
*		available at: www.trentonmize.com/software/cleanplots

*NOTE_4A: Option predict(outcome( )) allows for predictions from a specific
*        model after gsem. Models are named based on dependent variable

*NOTE_4B: Within pption predict(outcome( )) is where to specify which 
*		dependent variable category predictions are made

//  ologit

est restore gsemmodel
margins, at(age=(20(5)80)) atmeans ///
...
*save graph	
graph export "cmeS-4_5-A-ologit.png", replace
...
*save graph	
graph export "cmeS-4_5-B-mlogit.png", replace
