capture log close
log using cmeGS-4_2-Mediation-Logit-Happy, replace text

//  CME: Comparing Marginal Effects Across Models
//  Mize, Doan, and Long
//  Example 4.2: Mediation, binary logit, DV: being happy
...
qui mlincom 1-3, rowname(Diff in ADCs: M1 - M3) stat(est se p) add
qui mlincom 1-4, rowname(Diff in ADCs: M1 - M4) stat(est se p) add
qui mlincom 2-3, rowname(Diff in ADCs: M2 - M3) stat(est se p) add
qui mlincom 2-4, rowname(Diff in ADCs: M2 - M4) stat(est se p) add
mlincom 3-4, rowname(Diff in ADCs: M3 - M4) stat(est se p) add twidth(15)
